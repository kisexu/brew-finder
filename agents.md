...
Brew Finder is a Chrome extension (Manifest V3) that detects Homebrew packages for websites. It has three independent subsystems:
...
3. **Static website** (`site/`) — source files and locale content for the project website

All live in one repo. Both `data/` and the extension bundle at `extension/data/` are gitignored generated outputs; `npm run build:extension` copies the former into the latter. The website compiles to `dist/` (gitignored, generated by `npm run build:site`).
...
| File | Purpose |
|------|---------|
| `scripts/build-site.js` | Builds the static website from locale JSON into `dist/` |
| `site/styles.css` | Website styles, responsive layout, and hero background animation |
| `site/app.js` | Website interactions: copy buttons, language detection, language switching |
| `site/locales/*.json` | Per-language website content |
...
npm run build:site       # generate static website pages in dist/
npm run build            # both
```

See `docs/data-release.md` for reproducible local-input builds, validation thresholds, and the weekly artifact/manual Release workflow.

`npm run build:site` generates `dist/index.html` for English and `dist/<locale>/index.html` for every configured non-English locale. `dist/` is generated output and should not be committed.
...
The website is static HTML, but `site/app.js` handles language routing in the browser:

- On the homepage, it checks `navigator.languages`.
- It first tries an exact locale match, then a base-language fallback.
- Unsupported browser languages remain on the English homepage.
- Manual language choices are stored in `localStorage` under `brewFinder.siteLocale`.

To add a website language:

1. Add `site/locales/<locale>.json`.
2. Add the locale to `localeOrder` in `scripts/build-site.js` if display order matters.
