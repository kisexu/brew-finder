...
All live in one repo. The extension bundles the mapping data at `extension/data/` (gitignored, copied from `data/` by `npm run build:extension`). The website compiles to `dist/` (gitignored, generated by `npm run build:site`).
